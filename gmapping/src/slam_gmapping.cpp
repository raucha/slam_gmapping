...
- @b "scan"/<a
href="../../sensor_msgs/html/classstd__msgs_1_1LaserScan.html">sensor_msgs/LaserScan</a> : data from
...
Laser Parameters:
- @b "~/maxRange" @b [double] maximum range of the laser scans. Rays beyond this range get discarded
completely. (default: maximum laser range minus 1 cm, as received in the the first LaserScan
message)
- @b "~/maxUrange" @b [double] maximum range of the laser scanner that is used for map building
(default: same as maxRange)
...
- @b "~/iterations" @b [double] number of refinement steps in the scan matching. The final
"precision" for the match is lstep*2^(-iterations) or astep*2^(-iterations), respectively.
...
- @b "~/minimumScore" @b [double] minimum score for considering the outcome of the scanmatching
good. Can avoid 'jumping' pose estimates in large open spaces when using laser scanners with limited
range (e.g. 5m). (0 = default. Scores go up to 600+, try 50 for example when experiencing 'jumping'
estimate issues)
...
Others:
- @b "~/linearUpdate" @b [double] the robot only processes new measurements if the robot has moved
at least this many meters
- @b "~/angularUpdate" @b [double] the robot only processes new measurements if the robot has turned
at least this many rads

- @b "~/resampleThreshold" @b [double] threshold at which the particles get resampled. Higher means
more frequent resampling.
- @b "~/particles" @b [int] (fixed) number of particles. Each particle represents a possible
trajectory that the robot has traveled
...
bool SlamGMapping::initMapper(const sensor_msgs::LaserScan& scan) {
  ...
  gsp_->setMatchingParameters(maxUrange_, maxRange_, sigma_, kernelSize_, lstep_, astep_,
                              iterations_, lsigma_, ogain_, lskip_);
...
void SlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan) {
  ...
  if (addScan(*scan, odom_pose)) {
    ...
    // サブLRFの履歴に登録
    m_sub_scans_hist.insert(m_sub_scans_hist.begin() + 0, last_sub_scans);
...
void SlamGMapping::updateMap(const sensor_msgs::LaserScan& scan) {
  ...
  for (GMapping::GridSlamProcessor::TNode* n = best.node; n; n = n->parent) {
    ...
    for (int j = 0; j < SUB_LRF_NUM; j++) {
      ...
      if (do_reverse_range_) {
        ...
        for (int i = 0; i < num_ranges; i++) {
          // Must filter out short readings, because the mapper won't
          if (it->at(j).ranges[num_ranges - i - 1] < it->at(j).range_min)
            ranges_double[i] = (double)it->at(j).range_max;
          else
            ranges_double[i] = (double)it->at(j).ranges[num_ranges - i - 1];
        }
      } else {
        for (unsigned int i = 0; i < it->at(j).ranges.size(); i++) {
          // Must filter out short readings, because the mapper won't
          if (it->at(j).ranges[i] < it->at(j).range_min)
            ranges_double[i] = (double)it->at(j).range_max;
          else
            ranges_double[i] = (double)it->at(j).ranges[i];
        }
      }
  ...
  if (map_.map.info.width != (unsigned int)smap.getMapSizeX() ||
    ...
    // NOTE: The results of ScanMatcherMap::getSize() are different from the parameters given to
    // the
    // constructor
    ...
    ROS_DEBUG("map size is now %dx%d pixels (%f,%f)-(%f, %f)", smap.getMapSizeX(),
              smap.getMapSizeY(), xmin_, ymin_, xmax_, ymax_);
